import { Col, Image, Modal, Row, Switch, Upload } from 'antd'
...
function EditProduct() {
  ...
  const onFinish = async (values: IProduct) => {
    if(values.upload){
        ...
        try {
            await updateProduct({...values, id : dataItem.id, thumbnail : dataItem.thumbnail});
            popupSuccess(`Update product "${values.name}"  success`);
            handleCancel();
          } catch (err) {
            popupError(`Update prooduct "${values.name}"  error`);
            handleCancel();
          }
    }
  ...
  return (
    <>
      <Modal width={1400} okButtonProps={{ hidden: true }}  title={`Edit product "${dataItem?.name}"`} open={true} onCancel={handleCancel}>
        <Form
        ...
        >
          ...
          <Row gutter={[24, 8]}>
             <Col span={8}>
              ...
              <Col span={24}>
                    <Form.Item >
                        <Button loading={loadingUpdateProduct || file.loading} disabled={loadingUpdateProduct || file.loading} type='primary' htmlType='submit'>
                           Update
                        </Button>
                    </Form.Item>
              </Col>
